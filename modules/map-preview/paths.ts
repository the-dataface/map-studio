...
function createMarker(
	...
	const marker = defs.select<SVGMarkerElement>(`#${markerId}`).empty()
		? defs.append('marker').attr('id', markerId)
		: defs.select<SVGMarkerElement>(`#${markerId}`);
	...
	marker.selectAll<SVGElement, unknown>('*').remove();
	...
	switch (markerType) {
		...
		case 'square':
			// Filled square
			marker.append('rect').attr('x', 0).attr('y', 0).attr('width', size).attr('height', size).attr('fill', color);
			break;
		case 'round':
...
export function renderDrawnPaths({
	...
	const pathsGroup = svg.select<SVGGElement>('#DrawnPaths').empty()
		? svg.append<SVGGElement>('g').attr('id', 'DrawnPaths')
		: svg.select<SVGGElement>('#DrawnPaths');

	const paths = pathsGroup.selectAll<SVGGElement, DrawnPath>('g.path-group').data(drawnPaths, (d: DrawnPath) => d.id);
	...
	const pathGroups = paths
		...
		.attr('data-path-id', (d: DrawnPath) => d.id);

	// Merge enter and update
	const allPathGroups = paths.merge(pathGroups);

	// For each path group, render outline (if exists), main path, and markers
	allPathGroups.each(function (this: SVGGElement, d: DrawnPath) {
		const group = d3.select<SVGGElement, DrawnPath>(this);
		const pathId = d.id;
		...
		if (selectedPathId === pathId && onPathPointUpdate) {
			...
			const anchorPoints = group
				...
				.data(
					d.points.map((point: PathPoint, index: number) => ({ point, index, pathId })),
					(_: unknown, i: number) => `anchor-${pathId}-${i}`
				);
			...
			anchorPoints
				...
				.call(
					d3
						...
						.subject(function (
							event: d3.D3DragEvent<SVGCircleElement, { point: PathPoint; index: number; pathId: string }, unknown>,
							d: { point: PathPoint; index: number; pathId: string }
						) {
						...
						.on(
							...
							function (
								...
								const subject = event.subject as {
									x: number;
									y: number;
			...
			anchorPoints
				.attr('cx', (d: { point: PathPoint; index: number; pathId: string }) => d.point.x)
				.attr('cy', (d: { point: PathPoint; index: number; pathId: string }) => d.point.y);
	...
	if (activeTool === 'select' && onPathClick) {
		allPathGroups.selectAll('path[data-path-id]').on('click', function (event: MouseEvent) {
	...
	if (activeTool === 'move' && onPathPositionUpdate) {
		const drag = d3
			...
			.subject(function (event: d3.D3DragEvent<SVGPathElement, DrawnPath, unknown>, d: DrawnPath) {
				const [x, y] = d3.pointer(event, svg.node());
				return { x, y, originalPoints: d.points };
			})
			.on('start', function (event: d3.D3DragEvent<SVGPathElement, DrawnPath, unknown>) {
				if (event.sourceEvent) {
					event.sourceEvent.preventDefault();
					event.sourceEvent.stopPropagation();
				}
			...
			.on('drag', function (event: d3.D3DragEvent<SVGPathElement, DrawnPath, unknown>, d: DrawnPath) {
				if (event.sourceEvent) {
					event.sourceEvent.preventDefault();
					event.sourceEvent.stopPropagation();
				}
				...
				const updatedPoints = startPos.originalPoints.map((point: PathPoint) => ({
					...point,
					x: point.x + deltaX,
					y: point.y + deltaY,
			...
			.on('end', function (event: d3.D3DragEvent<SVGPathElement, DrawnPath, unknown>, d: DrawnPath) {
				if (event.sourceEvent) {
					event.sourceEvent.preventDefault();
					event.sourceEvent.stopPropagation();
				}
