...
export const getDefaultFormat = (
	type: 'number' | 'date' | 'state' | 'province' | 'county' | 'coordinate' | 'text' | 'country'
): string => {
	switch (type) {
		case 'number':
			return 'raw';
	...
	if (selectedGeography === 'canada-provinces') {
		...
		switch (format) {
			...
			case 'full':
				...
				return Object.values(PROVINCE_CODE_MAP).find((province) => province.toLowerCase() === str.toLowerCase()) || str;
			default:
				return str;
		}
	}
...
export const formatLegendValue = (
	...
	if (type === 'country') {
		// Use the formatCountry function from geography.ts
		// For now, return the value as-is since formatCountry is in geography.ts
